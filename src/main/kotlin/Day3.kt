...
private enum class Direction(val c: Char, val displacement: Pt) {
    UP('U', Pt(0, 1)),
    DOWN('D', Pt(0, -1)),
    LEFT('L', Pt(-1, 0)),
    RIGHT('R', Pt(1, 0));
    ...
    companion object {
        fun forChar(c: Char): Direction = values().first { it.c == c }
    }
}

private data class Move(val direction: Direction, val distance: Int) {
    companion object {
        fun fromString(s: String) = Move(Direction.forChar(s[0]), s.substring(1).toInt())
    }
}